...
@hydra.main(config_path=str(MODELS_PROJECT_ROOT / "conf"), config_name="csp", version_base="1.1")
def hydra_main(cfg: omegaconf.DictConfig):
    ...
    # 数据模块参数
    cfg.data_module.root_dir = DATASET_PATH  # 恢复数据集路径设置
    ...
    print(f"数据集路径: {DATASET_PATH}")  # 恢复数据集路径打印
...
def run_training():
    ...
    DATASET_PATH = str(PROJECT_ROOT / "datasets" / "cache" / DATA_MODULE_NAME)  # 恢复数据集路径设置
    ...
    print("+data_module.root_dir='{}'".format(DATASET_PATH))  # 恢复数据集路径打印
    ...
    # 再次确保环境变量设置
    os.environ["PROJECT_ROOT"] = str(PROJECT_ROOT)
    os.environ["MATTERGEN_MODELS_PROJECT_ROOT"] = str(PROJECT_ROOT)  # 恢复环境变量设置
